...
class _AuditProductsPageState extends State<AuditProductsPage>
  ...
  Future<void> _updateProductStatus(
    ...
    int auditId,
    int itemId,
    bool isNewProduct, // ✅ معامل جديد
  ) async {
    try {
      final key = '${auditId}_$itemId';
      _docIdMap[key] = docId;

      debugPrint('🔍 Product type: ${isNewProduct ? "NEW" : "EXISTING"}');
      debugPrint('🔍 Saving docId: $docId for key: $key');

      // ✅ إذا كان منتج جديد، لا نرسل للـ API
      if (!isNewProduct) {
        ...
        // ✅ للمنتجات الجديدة: تحديث مباشر في Firebase فقط
        debugPrint(
          '⚠️ New product - Skipping API call, updating Firebase only',
        );
            ...
            SnackBar(
              content: Row(
                children: [
                  ...
                  const Expanded(child: Text('تم تحديث الحالة بنجاح')),
                ],
              ),
  ...
  Widget build(BuildContext context) {
    return BlocProvider.value(
      ...
      child: Scaffold(
        ...
        // BlocListener<UpdateItemsStatusCubit, UpdateItemsStatusState>(
        //   listener: (context, state) async {
        //     if (state is UpdateItemsStatusLoaded) {
        //       final auditId = state.data?.auditId;
        //       final itemId = state.data?.itemId;
      ...
      builder: (context, snapshot) {
        ...
        return ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            final isNewProduct =
                data['is_new_product'] == true; // ✅ التحقق من المنتج الجديد
...
class CompactProductCard extends StatelessWidget {
  ...
  final bool isNewProduct; // ✅ معامل جديد
  ...
  const CompactProductCard({
    ...
    this.isNewProduct = false, // ✅ قيمة افتراضية
  });

  // ... باقي الكود بدون تعديل حتى build method
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        // ✅ إضافة border مختلف للمنتجات الجديدة
        border: isNewProduct
            ? Border.all(color: Colors.purple.shade300, width: 2)
            : null,
        boxShadow: [
          BoxShadow(
            color: isNewProduct ? Colors.purple.shade200 : Colors.grey.shade300,
            blurRadius: 10,
...
// ✅ استبدل CancelledProductsView class بالكامل:
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(12),
        border: Border.all(
          color: isNewProduct ? Colors.purple.shade300 : Colors.red.shade300,
          width: 2,
        ),
        boxShadow: [
          BoxShadow(
            color: isNewProduct ? Colors.purple.shade200 : Colors.grey.shade200,
            blurRadius: 6,
...
//   }
// }
// ✅ CompletedProductsView - لا يحتاج تعديل في التوقيع لأنه للعرض فقط
// لكن نحتاج تمرير isNewProduct لـ FullDetailsCard
        ...
        return ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            return FullDetailsCard(
              productData: data,
              status: status,
              isNewProduct: isNewProduct, // ✅ تمرير المعلومة
            );
          },
        );
      },
